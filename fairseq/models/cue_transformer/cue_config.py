...
_NAME_PARSER = r"(decoder|src_encoder|cxt_encoder|quant_noise)_(.*)"
...
class CUEConfig(FairseqDataclass):
    ...
    src_encoder: EncDecBaseConfig = EncDecBaseConfig()
    ...
    def _copy_keys(args, cls, prefix, seen):
        ...
        for fld in fields(cls):
            ...
            if "encoder" in fld.name:
                setattr(cfg, "encoder", safe_getattr(args, args_key))
        return cfg
    ...
    def from_namespace(cls, args):
        ...
        if not isinstance(args, cls):
            ...
            for fld in fields(cls):
                ...
                elif fld.name == "src_encoder":
                    if safe_hasattr(args, "encoder"):
                        seen.add("src_encoder")
                        config.src_encoder = EncDecBaseConfig(**args.src_encoder)
                    else:
                        config.src_encoder = cls._copy_keys(
                            args, EncDecBaseConfig, "src_encoder", seen
                        )
                elif fld.name == "cxt_encoder":
                    # same but for encoder
                    if safe_hasattr(args, "encoder"):
                        seen.add("cxt_encoder")
                        config.cxt_encoder = EncDecBaseConfig(**args.cxt_encoder)
                    else:
                        config.cxt_encoder = cls._copy_keys(
                            args, EncDecBaseConfig, "cxt_encoder", seen
                        )
